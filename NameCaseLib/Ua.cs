...
namespace NameCaseLib
{
    ...
    public class Ua : Core.Core
    {
        ...
        {
            if (In(letter, " " + consonant + vowels)) return false;
            return true;
        }
        ...
        private int detect2Group(string word)
        {
            ...
            if (stacksize > 0) last = stack.Substring(0, 1);

            var osnovaEnd = Last(osnova, 1);
            if (In(osnovaEnd, neshyplyachi) && !In(last, myaki))
                return 1;
            if (In(osnovaEnd, shyplyachi) && !In(last, myaki))
                return 2;
            return 3;
        }
        ...
        private string FirstLastVowel(string word, string vowels)
        {
            ...
            for (var i = length - 1; i > 0; i--)
            {
                ...
                if (In(letter, vowels)) return letter;
            }
        ...
        private string getOsnova(string word)
        {
            ...
            while (In(Last(osnova, 1), vowels + "ь")) osnova = osnova.Substring(0, osnova.Length - 1);
            return osnova;
        }
        ...
        protected bool ManRule1()
        {
            ...
            if (Last(1) == "а")
            {
                WordForms(workingWord,
                    new[]
                    {
                        beforeLast + "и", inverseGKH(beforeLast) + "і", beforeLast + "у", beforeLast + "ою",
                        inverseGKH(beforeLast) + "і", beforeLast + "о"
                    }, 2);
                Rule(101);
                return true;
            }
            ...
            if (Last(1) == "я")
                ...
                else
                {
                    WordForms(workingWord,
                        new[]
                        {
                            beforeLast + "і", inverseGKH(beforeLast) + "і", beforeLast + "ю", beforeLast + "ею",
                            inverseGKH(beforeLast) + "і", beforeLast + "е"
                        }, 2);
                    Rule(103);
                    return true;
                }
        ...
        protected bool ManRule3()
        {
            ...
            if (In(Last(1), consonant + "оь"))
            {
                ...
                if (osLast != "й" && Last(osnova, 2, 1) == "і" && !In(Last(osnova, 4), new[] {"світ", "цвіт"}) &&
                ...
                if (group == 1)
                    ...
                    else if (In(Last(2), new[] {"ов", "ев", "єв"}) && !InNames(workingWord, new[] {"лев", "остромов"}))
                    {
                        WordForms(osnova,
                            new[]
                            {
                                osLast + "а", osLast + "у", osLast + "а", osLast + "им", osLast + "у",
                                inverse2(osLast) + "е"
                            }, 1);
                        Rule(302);
                        return true;
                    ...
                    else
                    {
                        WordForms(osnova,
                            new[]
                            {
                                osLast + "а", osLast + "ові", osLast + "а", osLast + "ом", osLast + "ові",
                                inverse2(osLast) + "е"
                            }, 1);
                        Rule(304);
                        return true;
        ...
        protected bool WomanRule2()
        {
            if (In(Last(1), consonant + "ь"))
            {
                ...
                if (In(osLast, "мвпбф") && In(osbeforeLast, vowels)) apostrof = "’";

                //Чи треба подвоювати
                if (In(osLast, "дтзсцлн")) duplicate = osLast;
        ...
        protected bool WomanRule3()
        {
            ...
            if (Last(1) == "а" && (In(Last(2, 1), "чнв") || In(Last(3, 2), new[] {"ьк"})))
            {
                WordForms(workingWord,
                    new[]
                    {
                        beforeLast + "ої", beforeLast + "ій", beforeLast + "у", beforeLast + "ою", beforeLast + "ій",
                        beforeLast + "о"
                    }, 2);
                Rule(302);
                return true;
        ...
        protected override void GenderByFirstName(Word word)
        {
            ...
            if (Last(1) == "й") prob.Man += 0.9f;

            if (InNames(workingWord, new[] {"петро", "микола"})) prob.Man += 30;

            if (In(Last(2), new[] {"он", "ов", "ав", "ам", "ол", "ан", "рд", "мп", "ко", "ло"})) prob.Man += 0.5f;

            if (In(Last(3), new[] {"бов", "нка", "яра", "ила", "опа"})) prob.Woman += 0.5f;

            if (In(Last(1), consonant)) prob.Man += 0.01f;

            if (Last(1) == "ь") prob.Man += 0.02f;

            if (In(Last(2), new[] {"дь"})) prob.Woman += 0.1f;

            if (In(Last(3), new[] {"ель", "бов"})) prob.Woman += 0.4f;
        ...
        protected override void GenderBySecondName(Word word)
        {
            ...
            if (In(Last(2), new[] {"ов", "ин", "ев", "єв", "ін", "їн", "ий", "їв", "ів", "ой", "ей"})) prob.Man += 0.4f;

            if (In(Last(3), new[] {"ова", "ина", "ева", "єва", "іна", "мін"})) prob.Woman += 0.4f;

            if (In(Last(2), new[] {"ая"})) prob.Woman += 0.4f;
        ...
        protected override void GenderByFatherName(Word word)
        {
            ...
            if (Last(2) == "ич") word.GenderProbability = new GenderProbability(10, 0); // мужчина
            if (Last(2) == "на") word.GenderProbability = new GenderProbability(0, 12); // женщина
        }
        ...
        protected override void DetectNamePart(Word word)
        {
            ...
            if (In(Last(3), new[] {"вна", "чна", "ліч"}) || In(Last(4), new[] {"ьмич", "ович"})) father += 3;
            ...
            if (InNames(namepart,
                new[]
                {
                    "лев", "гаїна", "афіна", "антоніна", "ангеліна", "альвіна", "альбіна", "аліна", "павло", "олесь",
                    "микола", "мая", "англеліна", "елькін", "мерлін"
                })) first += 10;
            ...
            if (In(Last(2),
                new[]
                {
                    "ов", "ін", "ев", "єв", "ий", "ин", "ой", "ко", "ук", "як", "ца", "их", "ик", "ун", "ок", "ша",
                    "ая", "га", "єк", "аш", "ив", "юк", "ус", "це", "ак", "бр", "яр", "іл", "ів", "ич", "сь", "ей",
                    "нс", "яс", "ер", "ай", "ян", "ах", "ць", "ющ", "іс", "ач", "уб", "ох", "юх", "ут", "ча", "ул",
            ...
            if (In(Last(3),
                new[]
                {
                    "ова", "ева", "єва", "тих", "рик", "вач", "аха", "шен", "мей", "арь", "вка", "шир", "бан", "чий",
                    "іна", "їна", "ька", "ань", "ива", "аль", "ура", "ран", "ало", "ола", "кур", "оба", "оль", "нта",
                    "зій", "ґан", "іло", "шта", "юпа", "рна", "бла", "еїн", "има", "мар", "кар", "оха", "чур", "ниш",
            ...
            if (In(Last(4),
                new[]
                {
                    "ьник", "нчук", "тник", "кирь", "ский", "шена", "шина", "вина", "нина", "гана", "гана", "хній",
                    "зюба", "орош", "орон", "сило", "руба", "лест", "мара", "обка", "рока", "сика", "одна", "нчар",
                    "вата", "ндар", "грій"
                })) second += 0.4f;

            if (Last(1) == "і") second += 0.2f;
